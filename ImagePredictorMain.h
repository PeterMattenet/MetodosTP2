...
public:
	...
	void loadImagesFromFileDataSet(vector<string>& imageFileDataSet, vector<int>& etiquetas) {
	...
	int clasificarImagen(string image, int k){
	...
	int clasificarImagenSinPca(string image, int k){
		//Levanto la imagen, y la pongo en un vector de doubles
		//Z = vector de (mx1)
		Image img = Image(image);
		vector<double> imagePixels;
	...
	int clasificarImagenGrego(string image, int k){
		//Levanto la imagen, y la pongo en un vector de doubles
		//Z = vector de (mx1)
		Image img = Image(image);
		vector<double> imagePixels;
	...
	// usa Knn devolviendo la primer moda que se genera
	int clasificarImagenSinPcaGrego(string image, int k){
		//Levanto la imagen, y la pongo en un vector de doubles
		//Z = vector de (mx1)
		Image img = Image(image);
		vector<double> imagePixels;
	...
	int clasificarImagenPuntos(string image, int k){
		//Levanto la imagen, y la pongo en un vector de doubles
		//Z = vector de (mx1)
		Image img = Image(image);
		vector<double> imagePixels;
	...
	// usa K-puntos para clasificar
	int clasificarImagenSinPcaPuntos(string image, int k){
		//Levanto la imagen, y la pongo en un vector de doubles
		//Z = vector de (mx1)
		Image img = Image(image);
		vector<double> imagePixels;
	...
	int resolverKnn(vector<vector<double> >& MatrizDatos, vector<Image*>& imagenes, vector<double> z, int k){
		...
		*/
		...
		int vecinos = 0;
	 	...
	 	for(int i = 0 ; i < votaciones.size(); i ++){
	 		if(votaciones[i] != 0 ){
	 			...
	 			if(votacionDeLabel.find(labels[i]) == votacionDeLabel.end()){
	 				votacionDeLabel.insert(pair<int,int> (labels[i], 1));	
	 			}
	 			else{
	 				map<int,int>::iterator currentIt = votacionDeLabel.find(labels[i]);
	 				currentIt -> second += 1;
	 			}	
	 		}
	 	}
	 	...
	 	map<int,int>::iterator itRes = votacionDeLabel.begin();
	 	for(map<int,int>::iterator it = votacionDeLabel.begin(); it != votacionDeLabel.end() ; it ++){
	...
	//imagePixels es el vector imagen y K la cantidad de vecinos a considerar
	int resolverKnnGrego(vector<vector<double> >& MatrizDatos, vector<Image*>& imagenes, vector<double> z, int k){
		...
		*/
		...
		// en realidad votaciones[i] es 0 y 1, ya que o está entre las k más cercanas o no lo está.
	 	...
	 	for(int i = 0 ; i < labels.size(); i ++){
	 		if(votacionDeLabel.find(labels[i]) == votacionDeLabel.end()){
	 			votacionDeLabel.insert(pair<int,int> (labels[i], 0));		
	 		}
	 	}

	 	map<int,int>::iterator labelMasVotado = votacionDeLabel.begin();
	 	while(it != distancias.end() && vecinos < k ){
	 		map<int,int>::iterator currentLabel = votacionDeLabel.find(labels[it->second]);
	...
	int knnPorPuntos(vector<vector<double> >& MatrizDatos, vector<Image*>& imagenes, vector<double> z, int k){
		multimap<double,int> distancias;
 		for(int i = 0; i < n; i ++){
	  		distancias.insert(pair<double,int>(distanceBetweenVector(z,MatrizDatos[i]), i));
		}
